...
export class CreateProjectComponent {
  ...
  listOfControl: Array<{ id: number; controlInstance: string;controlInstance_value: string ;controlInstanceId:string}> = [];
  ...
  ngOnInit(): void {
    ...
    if (this.type != 'create'){
      this.projectService.getProject(this.p_id).subscribe((data)=>{
        ...
        this.validateForm.controls.teacher_point.setValue(this.project.teacher_grade_ratio);
        this.validateForm.controls.student_point_self.setValue(this.project.self_grade_ratio);
        this.validateForm.controls.student_point_mutual.setValue(this.project.mutual_grade_ratio);
        this.validateForm.controls.student_point_mutual.updateValueAndValidity();
        this.validateForm.controls.student_point_self.updateValueAndValidity();
      ...
      this.projectService.findGradeItemsByPid(this.p_id).subscribe((data)=>{
        ...
        this.items.forEach((item, index)=>{
          ...
          this.validateForm.addControl(control.controlInstance, new FormControl(item.description, Validators.required));
          this.validateForm.addControl(control.controlInstanceId, new FormControl(item.item_id, Validators.required));
          this.validateForm.addControl(control.controlInstance_value, new FormControl(item.max_grade, Validators.required));
  ...
  updateConfirmValidator(): void {
    ...
    Promise.resolve().then(() => this.validateForm.controls.teacher_point.updateValueAndValidity());
  }
  ...
  confirmValidator = (control: FormControl): { [s: string]: boolean } => {
    ...
    let total = (parseInt(this.validateForm.controls.teacher_point.value)
  ...
  addField(e?: MouseEvent): void {
    ...
    this.validateForm.addControl(control.controlInstanceId, new FormControl(-1, Validators.required));
    this.listOfControl.push(control);
  }

  removeField(i: { id: number; controlInstance: string ;controlInstance_value: string;controlInstanceId: string}, e: MouseEvent): void {
    e.preventDefault();
    // if (this.listOfControl.length > 1) {
