...
export class AdminProjectComponent implements OnInit {
  ...
  stopEdit(): void {
    ...
    this.projectService.changeProject(aproject,gradeItems).subscribe(
      (data)=>{
        if(data.code==200){
  ...
  search(): void {
    ...
    this.listOfDisplayData = this.listOfData.filter((item: ProjectData) => item.p_name.indexOf(this.searchValue) !== -1);
  }
  ...
  deleteProject(){
      ...
      this.listOfData =  this.listOfData.filter((data)=> data.p_id!==this.editId);
  ...
  getAllGradeItems(){
    this.adminService.getAllGradeItems().subscribe(
      (ItemData)=>{
